...
{#if providerInfo}
	<div class="space-y-6" style="transition: height 300ms ease-in-out;">
		...
		<div>
			...
			<div class="relative mt-1">
				<input
					...
					class="block w-full rounded-md border border-slate-300 px-3 py-2 text-sm placeholder-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
		...
		{#if providerInfo.models.length > 0}
			<div>
				<label for="model" class="block text-sm font-semibold text-slate-900"> Model </label>
				<select
					id="model"
					value={config.model}
					onchange={(e) => handleInputChange('model', e.currentTarget.value)}
					class="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
				>
					{#each providerInfo.models as model (model)}
		...
		{#if providerInfo.configFields.includes('endpoint')}
			<div>
				...
				<input
					...
					class="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm placeholder-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
		...
		<div class="flex items-center gap-4">
			<button
				...
				class={{
					...
					'cursor-not-allowed bg-slate-100 text-slate-600': connectionStatus === 'testing',
					'border border-emerald-200 bg-emerald-100 text-emerald-700':
						connectionStatus === 'success',
					'border border-red-200 bg-red-100 text-red-700': connectionStatus === 'error',

	...
	async testConnection(config: AIProviderConfig): Promise<ConnectionTestResult> {
		...
		try {
			...
			if (error && typeof error === 'object' && 'status' in error) {
				...
				switch (status) {
					case 401:
						return { success: false, error: 'Invalid Personal Access Token. Please check your token.' };
					case 403:
		...
		try {
			...
			if (error && typeof error === 'object' && 'status' in error) {
				...
				switch (status) {
					...
					case 500:
						throw new Error('GitHub Models service is temporarily unavailable. Please try again later.');
					case 502:
					case 503:
					case 504:
						throw new Error(
